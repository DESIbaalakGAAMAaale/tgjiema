 ...
 *
 * 矩阵 case:
 *   - case_id: loading_en_us_keyboard
 *   - state: loading
 *   - locale: en-US
 *   - input_mode: keyboard
 *
...
test('loading_en_us_keyboard: a11y matrix stub (route=/tasks, state=loading, locale=en-US, input_mode=keyboard)', async () => {
  // R65 P1-02: case path 引用(满足 scanner 路由引用校验)
  const routePath = '/tasks';
  const stateFixture = 'loading';
  ...
  expect(routePath).toBe('/tasks');
  expect(stateFixture).toBe('loading');
