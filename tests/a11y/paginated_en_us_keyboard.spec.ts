 ...
 *
 * 矩阵 case:
 *   - case_id: paginated_en_us_keyboard
 *   - state: paginated
 *   - locale: en-US
 *   - input_mode: keyboard
 *
...
test('paginated_en_us_keyboard: a11y matrix stub (route=/users, state=paginated, locale=en-US, input_mode=keyboard)', async () => {
  // R65 P1-02: case path 引用(满足 scanner 路由引用校验)
  const routePath = '/users';
  const stateFixture = 'paginated';
  ...
  expect(stateFixture).toBe('paginated');
