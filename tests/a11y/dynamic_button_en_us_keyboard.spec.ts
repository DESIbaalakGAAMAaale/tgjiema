 ...
 *
 * 矩阵 case:
 *   - case_id: dynamic_button_en_us_keyboard
 *   - state: dynamic_button
 *   - locale: en-US
 *   - input_mode: keyboard
 *
 * 路由/状态 fixture:
...
test('dynamic_button_en_us_keyboard: a11y matrix stub (route=/files, state=dynamic_button, locale=en-US, input_mode=keyboard)', async () => {
  // R65 P1-02: case path 引用(满足 scanner 路由引用校验)
  const routePath = '/files';
  const stateFixture = 'dynamic_button';
  ...
  expect(routePath).toBe('/files');
  expect(stateFixture).toBe('dynamic_button');
