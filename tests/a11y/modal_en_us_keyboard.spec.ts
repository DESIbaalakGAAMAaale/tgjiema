...
test('modal_en_us_keyboard: a11y matrix stub (route=/reports, state=modal, locale=en-US, input_mode=keyboard)', async () => {
  // R65 P1-02: case path 引用(满足 scanner 路由引用校验)
  const routePath = '/reports';
  const stateFixture = 'modal';
  ...
  expect(routePath).toBe('/reports');
  expect(stateFixture).toBe('modal');
