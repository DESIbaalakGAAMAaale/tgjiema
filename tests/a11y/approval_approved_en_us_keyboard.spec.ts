 ...
 *
 * 矩阵 case:
 *   - case_id: approval_approved_en_us_keyboard
...
test('approval_approved_en_us_keyboard: a11y matrix stub (route=/approvals, state=approval_approved, locale=en-US, input_mode=keyboard)', async () => {
  // R65 P1-02: case path 引用(满足 scanner 路由引用校验)
  const routePath = '/approvals';
  const stateFixture = 'approval_approved';
  ...
  expect(stateFixture).toBe('approval_approved');
