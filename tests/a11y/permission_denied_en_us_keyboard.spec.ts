 ...
 *
 * 矩阵 case:
 *   - case_id: permission_denied_en_us_keyboard
...
test('permission_denied_en_us_keyboard: a11y matrix stub (route=/rbac, state=permission_denied, locale=en-US, input_mode=keyboard)', async () => {
  // R65 P1-02: case path 引用(满足 scanner 路由引用校验)
  const routePath = '/rbac';
  const stateFixture = 'permission_denied';
  ...
  expect(routePath).toBe('/rbac');
  expect(stateFixture).toBe('permission_denied');
