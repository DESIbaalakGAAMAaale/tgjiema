...
test('empty_en_us_keyboard: a11y matrix stub (route=/notifications, state=empty, locale=en-US, input_mode=keyboard)', async () => {
  // R65 P1-02: case path 引用(满足 scanner 路由引用校验)
  const routePath = '/notifications';
  const stateFixture = 'empty';
  ...
  expect(routePath).toBe('/notifications');
  expect(stateFixture).toBe('empty');
