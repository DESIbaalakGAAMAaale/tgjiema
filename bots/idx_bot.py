...
async def _process_one_pending(app: Application, row: dict):
    ...
    if not file_types:
        ...
        logger.info(f"[Idx][poll] file_types 为空，尝试从 batch_file_meta 推断: type={type(batch_meta_raw).__name__}, value={batch_meta_raw!r}")
        if isinstance(batch_meta_raw, str) and batch_meta_raw:
            try:
                ...
                logger.info(f"[Idx][poll] 从 batch_file_meta(str) 推断 file_types: {file_types}")
            ...
            for m in batch_meta_raw:
                if isinstance(m, dict) and "type" in m:
                    file_types[m["type"]] = file_types.get(m["type"], 0) + 1
            logger.info(f"[Idx][poll] 从 batch_file_meta(list) 推断 file_types: {file_types}")
        else:
            logger.warning(f"[Idx][poll] batch_file_meta 无法用于推断: type={type(batch_meta_raw).__name__}")
    logger.info(f"[Idx][poll] 最终 file_types 用于生成文件码: {file_types}")
...
async def report_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ban_handle = await sign_button_token_with_handle(
            ...
            payload=f"ban|{uploader_id}|{reporter.id}|idx",
        )
        detach_handle = await sign_button_token_with_handle(
            principal_id=admin_principal_id, action="report",
            payload=f"detach|{file_code}|{reporter.id}|idx",
        )
        ...
        logger.error(f"[Idx][report] 生成签名按钮失败: {e}")
        await query.answer("举报提交失败,请稍后重试", show_alert=True)
        return

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(_i18n_t('bot.idx.s100'), callback_data=f"report:ban:{ban_handle}")],
        [InlineKeyboardButton(_i18n_t('bot.idx.s101'), callback_data=f"report:detach:{detach_handle}")],
        [InlineKeyboardButton(_i18n_t('bot.idx.s102'), callback_data=f"report:block:{block_handle}")],
...
async def _async_main():
    ...
    # CI 中 settings.DECODER_BOT_TOKEN 为空占位符 → .build() 抛 InvalidToken
    # → 进程崩溃 → Docker restart loop → docker compose exec 失败。
    # CI 模式跳过整个 Application 构建 + handler 注册 + bg_tasks,
    # 直接等待 stop 信号(与 up_bot/admin_bot RC31 一致)。
    _is_ci = (
    ...
    if _is_ci:
        ...
        logger.warning("[Idx] CI 模式: 跳过 Application 构建(占位符 token)")
        try:
            await stop_event.wait()
        except asyncio.CancelledError:
            pass
        finally:
            logger.info("[Idx] CI 模式关闭完成")
        return
