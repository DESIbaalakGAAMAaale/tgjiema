...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("menu:"):
        ...
        await safe_edit_message_text(query, UserMessage.from_raw_text(text), reply_markup=markup)
        return

    if data == "action:status":
        text = await _get_status_text()
        await safe_edit_message_text(query, UserMessage.from_raw_text(text), reply_markup=back_kb)
    ...
    # ─── 交互式操作入口 ──────────────────────────────────────────
        ...
        if entry:
            ...
            await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s87', action=action)), reply_markup=back_kb)
    ...
    # ─── 举报处理 ──────────────────────────────────────────────
        ...
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s88')), reply_markup=back_kb)
...
async def _handle_report_action(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data == "report:ignore":
        ...
        # 直接用新的 i18n 文案替换整个消息。
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s20')), reply_markup=None)
        return
    ...
    if len(parts) < 3:
        ...
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.expired_or_invalid')), reply_markup=None)
        return
    ...
    try:
        ...
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.signature_failed_report')), reply_markup=None)
        return
    if not valid:
        logger.warning(
    ...
    if len(payload_parts) < 1 or payload_parts[0] != sub_action:
        ...
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.payload_mismatch')), reply_markup=None)
        return
    ...
    try:
        ...
        if sub_action == "ban":
            if len(payload_parts) < 2:
                await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.missing_uid')), reply_markup=None)
                return
            try:
                uid = int(payload_parts[1])
            except ValueError:
                await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.invalid_uid')), reply_markup=None)
                return
            ...
            if result.approval_required:
                ...
                await safe_edit_message_text(query, UserMessage.from_raw_text(render_for_send(
            ...
            if len(payload_parts) < 2:
                await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.missing_file_code')), reply_markup=None)
                return
            ...
            if len(payload_parts) < 3:
                await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.missing_file_code_reporter')), reply_markup=None)
                return
            file_code = payload_parts[1]
            try:
                reporter_id = int(payload_parts[2])
            except ValueError:
                await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.invalid_reporter_id')), reply_markup=None)
                return
            ...
            await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.unknown_sub_action', sub_action=sub_action)), reply_markup=None)
    except Exception as e:
        # R64 P1-06: 内部异常不进用户面 params;仅记录结构化日志,
        # 用户面只暴露 trace_id 供引用。
        trace_id = uuid.uuid4().hex
...
async def _notify_reporter(reporter_id_str: str, source_bot: str, message: str):
    ...
    try:
        async with Bot(token=token) as bot:
            await safe_send_message(bot, payload=UserMessage.from_raw_text(message), chat_id=reporter_id)
    except Exception as e:
...
async def _handle_restore_action(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data == "restore:cancel":
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s8')), reply_markup=back_kb)
        return
    ...
    if not data.startswith("restore:confirm:"):
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s9')), reply_markup=back_kb)
        return
    ...
    try:
        ...
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.signature_failed_restore')), reply_markup=back_kb)
        return
    if not valid:
        logger.warning(
    ...
    parts = payload.split("|")
    if len(parts) < 1:
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s10')), reply_markup=back_kb)
        return

    try:
        seq = int(parts[0])
    except ValueError:
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s21')), reply_markup=back_kb)
        return
    ...
    await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s3', mode_label=mode_label)))

    from services.db_backup import list_backups
    try:
        backups = await list_backups()
    except Exception as e:
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s22', e=e)), reply_markup=back_kb)
        return

    if not backups or seq < 1 or seq > len(backups):
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s11')), reply_markup=back_kb)
        return
    ...
    try:
        ...
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s89', error=e, backup_key=key)), reply_markup=back_kb)
        return

    if result.approval_required:
        # 灾备恢复必须审批,告知用户审批 ID
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s12', mode_label=mode_label, result_approval_id=result.approval_id, key=key, mode_label_4=mode_label, user_id=user.id)), reply_markup=back_kb)
        return

    if result.success:
        # 不应到达此处(restore_backup 必须审批),但保持健壮性
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s13', key=key)), reply_markup=back_kb)
    else:
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s14', result_error=result.error, key=key)), reply_markup=back_kb)
...
async def _handle_delete_file_action(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data.startswith("delfile_cancel|"):
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s16')), reply_markup=back_kb)
        return
    ...
    parts = data.split("|", 1)
    if len(parts) < 2:
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s17')), reply_markup=back_kb)
        return
    handle_id = parts[1]
    ...
    try:
        ...
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('admin.callback.button_security.signature_failed_delete')), reply_markup=back_kb)
        return
    if not valid:
        logger.warning(
    ...
    if cb_result.approval_required:
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s18', cb_result_approval_id=cb_result.approval_id, file_code=file_code)), reply_markup=back_kb)
        return
    if not cb_result.success:
        await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s19', cb_result_error=cb_result.error)), reply_markup=back_kb)
        return
    # 失效缓存(handler 已更新 CRDB,此处仅清进程内缓存)
    try:
        invalidate_file_record(file_code)
    except Exception:
        logger.exception(_i18n_t('diagnostics.r65.p1_04.swallowed_exception', file_func='bots/admin_bot/callback.py:_handle_delete_file_action'))
    await safe_edit_message_text(query, UserMessage.from_raw_text(_i18n_t('bot.admin_bot.callback.s4', file_code=file_code)), reply_markup=back_kb)